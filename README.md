...
- Create a copy of ``github_monit/settings/local.py.example``:  
  `cp github_monit/settings/local.py.example github_monit/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/github_monit/settings/base.py``.
- Adjust ``backend/github_monit/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
